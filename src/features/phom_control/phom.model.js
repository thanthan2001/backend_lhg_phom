...
//       console.log("==============CAP NHAT STATE AND ID  PHIEU MUON CHI TIET==========================");
...
//     const LO = await db.Execute(companyname, `EXEC sp_GenerateLastInOutNo`);
//     const LastInOutNo = LO.jsonArray[0].NewLastInOutNo;
//     const LastSumQty = payload.BILL_BORROW.scannedRfidDetailsList.reduce((acc, item) => {
//       return acc + item.LastSum;
//     }).parseFloat();
//     await db.Execute(companyname, `
...
//       '${LastInOutNo}', ${parseFloat(LastSumQty/2)}, '${payload.userId}', GETDATE(), 'Y', '${payload.BILL_BORROW.RFIDDetails[0].LastMatNo}'
//       )
//     `);
...
//         '${LastInOutNo}', '${item.LastSize}', ${item.LastSum}, 'Y', '${payload.userId}', GETDATE(), 'ZZZZ', '${item.LastMatNo}'
//         )
//       `);
//     }
...
//     const LO_Return = await db.Execute(companyname, `EXEC sp_GenerateLastInOutNo`);
...
//     await db.Execute(companyname, `
//       INSERT INTO LastInOut_M (LastInOutNo, LastInOutQty, CreID, CreDate, YN, LastMatNo) VALUES(
//       '${LastInOutNoReturn}', ${parseFloat((LastSumQtyReturn/2))}, '${payload.userId}', GETDATE(), 'Y', '${payload.BILL_RETURN[0].LastMatNo}'
//       )
//     `);
//     for (const item of payload.BILL_RETURN) {
//       await db.Execute(companyname, `
//         INSERT INTO LastInOut_D (LastInOutNo, LastSize, LastQty, YN, CreID, CreDate, Country, LastMatNo) VALUES(
//         '${LastInOutNoReturn}', '${item.LastSize}', ${item.LastSum}, 'Y', '${payload.userId}', GETDATE(), 'ZZZZ', '${item.LastMatNo}'
//         )
//       `);
//     };
...
//           LastSumQty: LastSumQty,
//           LastSumQtyReturn: LastSumQtyReturn,
//         }
//       ],
...
for (const item of payload.BILL_BORROW.scannedRfidDetailsList) {
  await db.Execute(companyname, `
    INSERT INTO LastInOut_D (LastInOutNo, LastSize, LastQty, YN, CreID, CreDate, Country, LastMatNo) VALUES(
    '${LastInOutNo}', '${item.LastSize}', ${Number(item.LastSum) || 0}, 'Y', '${payload.userId}', GETDATE(), 'ZZZZ', '${item.LastMatNo}'
    )
  `);
}
console.log("==============XONG TẠO PHIẾU MƯỢN==========================");
...
if (LastInOutNo === LastInOutNoReturn) {
    console.error("CRITICAL ERROR: sp_GenerateLastInOutNo returned the same ID for borrow and return operations. This will cause PK violations.");
...
for (const item of payload.BILL_RETURN) {
  await db.Execute(companyname, `
    INSERT INTO LastInOut_D (LastInOutNo, LastSize, LastQty, YN, CreID, CreDate, Country, LastMatNo) VALUES(
    '${LastInOutNoReturn}', '${item.LastSize}', ${Number(item.LastSum) || 0}, 'Y', '${payload.userId}', GETDATE(), 'ZZZZ', '${item.LastMatNo}'
    )
  `);
};
